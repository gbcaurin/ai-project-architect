...
async def extract_project_data(conversation: list) -> dict:
    ...
    try:
        result = await llm.ainvoke(EXTRACTION_PROMPT.format(conversation=conv_text))
        content = result.content if hasattr(result, "content") else str(result)
        content = content.strip().lstrip("```json").lstrip("```").rstrip("```").strip()
        return json.loads(content)
    except Exception:
        from mock_llm import EXTRACTION_TEMPLATE
...
async def analyze_project(project_data: dict) -> dict:
    ...
    try:
        result = await llm.ainvoke(ANALYSIS_PROMPT.format(project_data=json.dumps(project_data, ensure_ascii=False, indent=2)))
        content = result.content if hasattr(result, "content") else str(result)
        content = content.strip().lstrip("```json").lstrip("```").rstrip("```").strip()
        return json.loads(content)
    except Exception:
        from mock_llm import ANALYSIS_TEMPLATE
