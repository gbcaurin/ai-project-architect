...
export function PromptPipelineView({
  selectedAI,
  onSelectAI,
  pipeline,
  isGenerating,
      ...
      {isGenerating && !pipeline ? (
        <div className="flex flex-col items-center justify-center h-48 gap-4">
          ...
          <p className="text-sm text-gray-500">
            Gerando prompts para {selectedAI}…
          </p>
        </div>
      ) : !pipeline ? (
        <div className="flex flex-col items-center justify-center h-48 gap-4 text-center">
          <p className="text-sm text-gray-400 font-medium">
            Nenhum prompt ainda para {selectedAI}
          </p>
          <button
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs text-gray-600">
              {pipeline.prompts.length} etapas no pipeline — clique em qualquer
              etapa para expandir
            </p>
            <button
              onClick={() => onGenerate(selectedAI)}
