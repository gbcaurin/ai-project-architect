...
interface Props {
  projectId: string;
  onInterviewComplete?: () => void; // ← adiciona
}

export function InterviewPanel({ projectId, onInterviewComplete }: Props) {
  ...
  // ← dispara quando entrevista completa
  useEffect(() => {
    if (interviewComplete && onInterviewComplete) {
      onInterviewComplete();
    }
  }, [interviewComplete]);
        ...
        {interviewComplete ? (
          <div className="text-center py-3">
            <p className="text-xs text-gray-500">
              Entrevista concluída — confira o painel de resultados →
            </p>
          </div>
        ) : (
          <ChatInput
